...
func (f decimalFormat) leadingZeros(s string) int {
    zeros := 0
    for _, c := range s {
        zero, ok := decimalRuneToInt(c, &f.Digits)
        if !ok { continue }
        if zero == 0 { zeros++; continue }
        return zeros
    }
    return 0
}
...
func (f decimalFormat) AcceptFloat(s string) (value float64, length int, err error) {
    ...
    if pointLen > 0 && (s[leftLen +pointLen] != '-') {
        ...
        leadingZeros = f.leadingZeros(s[leftLen +pointLen:])
    ...
    if right > 0.0 {
        ...
        places := 1.0 + math.Floor(math.Log10(fRight)) + float64(leadingZeros)
